...
export default function Home() {
  ...
  const householdOptions = Array.from({ length: 20 }, (_, i) => i + 1);

  function formatCurrency(value) {
    return `$${Math.round(value).toLocaleString()}`;
  }

  function convertAmount(value, view) {
    if (view === "monthly") return value / 12;
    if (view === "weekly") return value / 52;
    return value;
  }

  const benchmarkData = useMemo(() => {
    const stateMultiplier = state === "NY" ? 1.08 : 1;

    const highIncomeBase = 110000 + householdSize * 15000;
    const medianIncomeBase = 78000 + householdSize * 11000;
    const lowIncomeBase = 50000 + householdSize * 7500;

    const tuitionHigh = householdSize <= 2 ? 0 : (householdSize - 2) * 15500;
    const tuitionMedian = householdSize <= 2 ? 0 : (householdSize - 2) * 10500;
    const tuitionLow = householdSize <= 2 ? 0 : (householdSize - 2) * 6500;

    const annualRows = [
      {
        category: "Housing",
        high: (18000 + householdSize * 3200) * stateMultiplier,
        median: (14000 + householdSize * 2400) * stateMultiplier,
        low: (11000 + householdSize * 1800) * stateMultiplier,
      },
      {
        category: "Food & groceries",
        high: (5000 + householdSize * 2300) * stateMultiplier,
        median: (4200 + householdSize * 1900) * stateMultiplier,
        low: (3400 + householdSize * 1500) * stateMultiplier,
      },
      {
        category: "Tuition & childcare",
        high: tuitionHigh * stateMultiplier,
        median: tuitionMedian * stateMultiplier,
        low: tuitionLow * stateMultiplier,
      },
      {
        category: "Debt & savings",
        high: (7000 + householdSize * 3000) * stateMultiplier,
        median: (4000 + householdSize * 1800) * stateMultiplier,
        low: (1800 + householdSize * 900) * stateMultiplier,
      },
      {
        category: "Healthcare",
        high: (2200 + householdSize * 1200) * stateMultiplier,
        median: (1800 + householdSize * 950) * stateMultiplier,
        low: (1400 + householdSize * 700) * stateMultiplier,
      },
      {
        category: "Transportation",
        high: (4000 + householdSize * 1400) * stateMultiplier,
        median: (3200 + householdSize * 1150) * stateMultiplier,
        low: (2500 + householdSize * 850) * stateMultiplier,
      },
      {
        category: "Clothing / simchas / misc.",
        high: (4200 + householdSize * 2100) * stateMultiplier,
        median: (3200 + householdSize * 1700) * stateMultiplier,
        low: (2200 + householdSize * 1200) * stateMultiplier,
      },
    ];

    return {
      highIncome: convertAmount(highIncomeBase * stateMultiplier, timeView),
      medianIncome: convertAmount(medianIncomeBase * stateMultiplier, timeView),
      lowIncome: convertAmount(lowIncomeBase * stateMultiplier, timeView),
      rows: annualRows.map((row) => ({
        category: row.category,
        high: convertAmount(row.high, timeView),
        median: convertAmount(row.median, timeView),
        low: convertAmount(row.low, timeView),
      })),
    };
  }, [householdSize, state, timeView]);
  ...
  const faqItems = [
    [
      "Do we need to be in crisis before reaching out?",
      "Not at all. Many families come because they want structure before the pressure grows further.",
    ],
    [
      "Will you force us into a strict system?",
      "No. The point is to build around your actual numbers, values, and constraints.",
    ],
    [
      "Is this only for families with debt?",
      "No. Some need help with debt, while others want more clarity around cash flow, planning, and monthly structure.",
    ],
    [
      "Do both spouses need to be involved?",
      "Whenever possible, yes. The strongest results usually happen when both spouses understand the plan and feel part of it.",
    ],
  ];
    ...
    <div className="min-h-screen bg-[#FBF8F3] text-[#1D2834]">
      <header className="sticky top-0 z-50 border-b border-[#E9DFD3] bg-[rgba(251,248,243,0.94)] backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-5 md:px-10 lg:px-14">
          <div>
            <div className="text-2xl font-semibold tracking-tight">Henig Financial</div>
        ...
        <div className="mx-auto grid max-w-7xl gap-10 px-6 py-10 md:px-10 lg:grid-cols-[0.95fr_1.05fr] lg:items-center lg:px-14 lg:py-16">
          <div className="max-w-2xl">
            ...
            <p className="mt-7 max-w-xl text-lg leading-8 text-[#5F6977] md:text-xl">
              We help families build calm, structure, and forward movement by working
              from their actual numbers, actual pressure points, and actual goals.
            </p>

            <p className="mt-5 max-w-xl text-base leading-7 text-[#445060]">
              No generic systems. No financial shame. Just thoughtful guidance shaped
              around the way your family actually lives.
            </p>
            ...
            <div className="overflow-hidden rounded-[38px] border border-[#E8DED2] bg-[linear-gradient(135deg,#F1E7DB_0%,#E5D4C0_45%,#D1B093_100%)] shadow-[0_22px_54px_rgba(29,40,52,0.1)]">
              <div className="grid min-h-[620px] grid-rows-[1fr_auto]">
                ...
                <div className="grid gap-4 border-t border-white/30 bg-[rgba(255,255,255,0.28)] px-6 py-6 md:grid-cols-3 md:px-8">
                  {[
                    ["Awareness first", "Understand where the pressure really lives."],
                    ["Benchmarks with dignity", "Context without comparison or shame."],
                    ["Guidance that holds", "A plan your family can actually live with."],
                  ].map(([title, text]) => (
                    <div
                      key={title}
                    ...
                    >
                      ...
                      <div className="mt-2 text-sm leading-6 text-[#4C5866]">{text}</div>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <section className="bg-[#F5EFE7]">
        <div className="mx-auto grid max-w-7xl gap-6 px-6 py-8 md:grid-cols-4 md:px-10 lg:px-14">
          {[
            ["Tailored family guidance", "We build around your life, not a template."],
            ["Real numbers, real clarity", "We work from actual income, expenses, and pressure points."],
            ["Practical plans, not pressure", "The goal is progress your family can actually sustain."],
            ...
            <div className="grid gap-6 lg:grid-cols-[1.02fr_0.98fr]">
              <div className="rounded-[34px] border border-[#E7DDD1] bg-white p-8 shadow-[0_16px_34px_rgba(29,40,52,0.05)] md:p-10">
                <p className="text-lg leading-8 text-[#5F6977]">
                  At Henig Financial, financial guidance begins with understanding, not assumptions.
                  Before building any plan, we step into your family’s reality—income, obligations,
                  habits, goals, and pressure points—so the path feels more honest, more tailored,
              ...
              <div className="rounded-[34px] border border-[#E7DDD1] bg-[#1F3448] p-8 text-white shadow-[0_16px_34px_rgba(29,40,52,0.08)] md:p-10">
                ...
                <div className="space-y-5">
                  {[
                    ["Before", "Overwhelm, uncertainty, emotional spending, and money tension."],
                    ["Middle", "Clarity, tradeoffs, better communication, and practical priorities."],
                    ["After", "A steadier plan, more confidence, and a calmer home."],
                  ].map(([label, text]) => (
                    <div
                      key={label}
        ...
        <div className="mx-auto max-w-7xl px-6 py-24 md:px-10 lg:px-14">
          <div className="mx-auto max-w-5xl text-center">
            ...
            <p className="mt-5 text-lg leading-8 text-[#5F6977]">
              How similar households typically earn and spend across key categories.
            </p>
          </div>
            ...
            <div className="grid xl:grid-cols-[1.18fr_0.82fr]">
              <div className="border-r border-[#ECE2D6] p-6 md:p-8 lg:p-10">
                <div className="grid gap-0 overflow-hidden rounded-[28px] border border-[#ECE2D6] md:grid-cols-3">
                  <div className="bg-[#203A53] p-5 text-white">
                    ...
                    <div className="mt-3 text-[30px] font-semibold tracking-tight leading-none">
                      {formatCurrency(benchmarkData.highIncome)}
                    </div>
                  ...
                  <div className="bg-[#B97854] p-5 text-white">
                    ...
                    <div className="mt-3 text-[30px] font-semibold tracking-tight leading-none">
                      {formatCurrency(benchmarkData.medianIncome)}
                    </div>
                    <div className="mt-2 text-sm opacity-80">{timeLabel}</div>
                  </div>
                  ...
                  <div className="bg-[#D5B28E] p-5 text-[#2D241E]">
                    ...
                    <div className="mt-3 text-[30px] font-semibold tracking-tight leading-none">
                      {formatCurrency(benchmarkData.lowIncome)}
                    </div>
                    <div className="mt-2 text-sm opacity-80">{timeLabel}</div>
                  </div>
                </div>
                  ...
                  <table className="min-w-full text-left">
                    ...
                    <tbody className="text-[#495563]">
                      {benchmarkData.rows.map((row, idx) => (
                        <tr key={row.category} className={idx % 2 === 0 ? "bg-white" : "bg-[#FBF7F1]"}>
                          <td className="px-5 py-4 font-medium text-[#1D2834]">{row.category}</td>
                          <td className="px-5 py-4">{formatCurrency(row.high)}</td>
                          <td className="px-5 py-4">{formatCurrency(row.median)}</td>
                          <td className="px-5 py-4">{formatCurrency(row.low)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <div className="mt-4 text-sm text-[#66707E]">
                  Benchmarks are guides, not rules. They are meant to build awareness, not pressure.
                </div>
              </div>
              ...
              <div className="bg-[linear-gradient(180deg,#FCF8F3_0%,#F5EDE3_100%)] p-6 md:p-8 lg:p-10">
                <div className="rounded-[30px] border border-[#E7DACB] bg-white/75 p-6 shadow-sm">
                  ...
                  <p className="mt-4 leading-7 text-[#5F6977]">
                    Most families do not need more pressure. They need honest context.
                    This section helps visitors understand where pressure may be coming from
                    and which categories deserve closer attention.
                  </p>

                  <div className="mt-8 space-y-4">
                    {[
                      ["Housing", "One of the largest anchors in the family budget."],
                      ["Tuition & childcare", "For some households this may be zero, and for others it may be one of the biggest line items."],
                      ["Seasonal life", "Yom Tov, simchas, camps, and clothing rarely fit neatly into one line item."],
                    ].map(([title, text]) => (
                      <div
                        key={title}
                        className="rounded-2xl border border-[#EBDDCF] bg-[#FCFAF7] p-4"
                      >
                        <div className="font-semibold text-[#1D2834]">{title}</div>
                        <div className="mt-1 text-sm leading-6 text-[#61707B]">{text}</div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="mt-8 rounded-[24px] bg-[#1F3448] p-5 text-white">
                    ...
                    <div className="mt-3 text-lg leading-7 text-white/82">
                      We use these benchmarks to start better conversations — then build around
                      your family’s real picture, not the averages.
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="mt-24 grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
            <div>
              ...
              <p className="mt-5 text-lg leading-8 text-[#5F6977]">
                This section makes the website feel useful, thoughtful, and grounded in reality
                rather than generic coaching language.
              </p>
            </div>
            ...
            <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
              {[
                ...
                ["Tuition & children", "Day school, childcare, camp, and activities."],
                ["Seasonal life", "Yom Tov, simchas, hosting, and travel."],
                ["Debt pressure", "Credit cards, loans, and monthly drag."],
                ["Savings & stability", "Emergency fund, reserves, and longer goals."],
              ].map(([title, text]) => (
                <div
                  key={title}
        ...
        <div className="mx-auto max-w-7xl px-6 py-24 md:px-10 lg:px-14">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="mt-5 text-lg leading-8 text-white/76">
              A simple structure that helps families move from fog and reaction to clarity and steady progress.
            </p>
          </div>
          ...
          <div className="mt-12 grid gap-6 lg:grid-cols-3">
            {[
              ...
              ["Change", "Together we build a realistic plan around your life."],
              ["Maintenance", "We refine, adjust, and support the progress over time."],
            ].map(([title, text]) => (
              <div
                key={title}
        ...
        <div className="mx-auto max-w-7xl px-6 py-24 md:px-10 lg:px-14">
          <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
            <div>
              ...
              <p className="mt-5 text-lg leading-8 text-[#5F6977]">
                A premium resource layer makes the site feel more valuable than a brochure
                and more helpful before the first conversation.
              </p>
            </div>
            ...
            <div className="grid gap-6 md:grid-cols-2">
              {resources.map((resource) => (
                ...
                >
                  ...
                  <h3 className="text-xl font-semibold text-[#1D2834]">{resource.title}</h3>
                  <p className="mt-3 leading-7 text-[#5F6977]">{resource.text}</p>
      ...
      <section id="stories" className="bg-[#FBF8F3]">
        <div className="mx-auto max-w-7xl px-6 py-24 md:px-10 lg:px-14">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="mt-5 text-lg leading-8 text-[#5F6977]">
              Even before full testimonials, the site can show the kind of outcomes families are looking for.
            </p>
          </div>

          <div className="mt-12 grid gap-6 lg:grid-cols-3">
            {stories.map(([quote, label, outcome]) => (
              <div
                key={label}
                className="rounded-[30px] border border-[#E8DED2] bg-white p-7 shadow-[0_14px_30px_rgba(29,40,52,0.06)]"
              >
                <div className="mb-4 text-4xl leading-none text-[#D3A27E]">“</div>
                <p className="text-lg leading-8 text-[#45515F]">{quote}</p>
                <div className="mt-6 border-t border-[#EFE5D8] pt-4">
                  <div className="font-semibold text-[#1D2834]">{label}</div>
                  <div className="text-sm text-[#7C8795]">{outcome}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="mx-auto max-w-7xl px-6 py-24 md:px-10 lg:px-14">
          ...
          <div className="mx-auto mt-12 max-w-4xl space-y-4">
            {faqItems.map(([q, a]) => (
              ...
              >
                <div className="text-xl font-semibold text-[#1D2834]">{q}</div>
                <div className="mt-3 leading-7 text-[#5E6977]">{a}</div>
              </div>
            ))}
          </div>
        </div>
            ...
            <div className="grid gap-8 lg:grid-cols-[1.15fr_0.85fr] lg:items-center">
              <div>
                ...
                <div className="text-4xl font-semibold tracking-tight text-white md:text-5xl">
                  Let’s bring clarity, structure, and peace back into your family finances.
                </div>
                <p className="mt-5 max-w-2xl text-lg leading-8 text-white/76">
                  A first conversation can help you understand where things stand,
                  what is creating pressure, and what kind of path may make the most sense from here.
                </p>
              </div>
              ...
              <div className="rounded-[30px] bg-[#F7F0E7] p-6 text-[#1D2834] shadow-xl">
                ...
                <p className="mt-2 leading-7 text-[#5D6876]">
                  A warm and practical first conversation to understand your situation
                  and the support that would help most.
                </p>
        ...
        <div className="mx-auto grid max-w-7xl gap-10 px-6 py-16 md:grid-cols-4 md:px-10 lg:px-14">
          <div>
            ...
            <p className="mt-4 leading-7 text-white/68">
              Tailored financial guidance for families who want clear numbers, calmer decisions,
              and a path they can actually live with.
            </p>
          </div>
          ...
          <div>
            ...
            <div className="mt-4 space-y-3 text-white/74">
              ...
              <a href="mailto:info@henigfinancial.com" className="block hover:text-white">
          ...
          <div>
            ...
            <p className="mt-4 leading-7 text-white/68">
              Benchmarks shown on this website are for awareness and educational guidance.
              Individual plans are always built around each family’s real numbers and real goals.
            </p>
          </div>
        </div>
      </footer>
